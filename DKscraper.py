...
def overarching_method(url, csvOutFileName):
    ...
    print(f"Scraping for eventId {eventId}...")
    df = scrape_DK_event(eventId)
    if csvOutFileName is None:
        csvOutFileName = f"dk_scrape_{eventId}.csv"
    print(f"Writing to {csvOutFileName}")
...
def main():
    ...
    parser.add_argument('url', type=str, help='This is a required argument.')
    parser.add_argument('csvOutFileName', type=str, nargs='?', default=None,
                        help='This is an optional argument. If not provided, it will have a default value.')
